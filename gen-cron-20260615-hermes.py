#!/usr/bin/env python3
"""Generate 5 Dutch consumer comparison articles — June 15, 2026 cron. V4: delimiter approach."""
...
def load_api_key():
    ...
    with open(env_path) as f:
        for line in f:
            if line.startswith("GEMINI_API_KEY=***                return line.strip().split("=", 1)[1]
    return None
...
def parse_response(raw):
    ...
    if len(parts) < 2:
        ...
        # Fall back to full JSON parse with newline fixing
        fixed = []
        in_str = False
        esc = False
        for ch in raw:
...
ARTICLES = [
    ("beste-budget-apps-2026-dyme-spendle-ynab-wallet-grip", "de beste budget apps en geldbeheer tools voor Nederlanders: huishoudboekje, uitgaven tracken en sparen", "persoonlijk", "Nederlanders die grip willen op hun uitgaven en spaardoelen", "Dyme, Spendle, YNAB, Wallet by BudgetBakers, Grip (ABN AMRO), Flow Your Money, MijnGeldzaken"),
    ("beste-online-banken-2026-bunq-revolut-n26-knab-asn", "de beste online banken en neobanken voor Nederlanders: digitaal bankieren, spaarrentes en betaalpakketten", "persoonlijk", "Nederlanders die overstappen naar een moderne online bank", "bunq, Revolut, N26, Knab, ASN Bank, Openbank, Trade Republic"),
    ("beste-reisverzekering-2026-anwb-allianz-europeesche-ohra-unive", "de beste doorlopende reisverzekeringen: dekking, werelddekking, annulering en medische kosten", "persoonlijk", "Nederlandse reizigers die een doorlopende reisverzekering zoeken", "ANWB, Allianz Global Assistance, Europeesche, OHRA, Univé, Centraal Beheer, FBTO"),
    ("beste-internetproviders-2026-ziggo-kpn-odido-delta-tmobile", "de beste internetproviders voor thuis: glasvezel, DSL en kabel vergeleken op snelheid, prijs en betrouwbaarheid", "huis-tuin", "Nederlandse huishoudens die een internetabonnement zoeken", "Ziggo, KPN, Odido, Delta Glasvezel, T-Mobile Thuis, Youfone Internet, Freedom Internet"),
    ("beste-zorgverzekering-2026-zilveren-kruis-cz-vgz-menzis-dsw", "de beste zorgverzekeringen: basisverzekering, aanvullende pakketten en tandartsverzekering vergeleken", "persoonlijk", "Nederlanders die hun zorgverzekering willen vergelijken voor 2026", "Zilveren Kruis, CZ, VGZ, Menzis, DSW, FBTO Zorg, Ditzo"),
]
