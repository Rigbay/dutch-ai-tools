#!/usr/bin/env python3
"""Retry 2 failed articles — V5: body_markdown inside JSON field, no delimiter."""
...
def parse_response(raw):
    ...
    try:
        ...
        print(f"  JSON parse failed at pos {e.pos}: {''.join(fixed)[max(0,e.pos-40):e.pos+40]}")
        return None
...
OUTPUT: Alleen een JSON object, géén markdown fences. Het body_markdown veld bevat de volledige artikeltekst in Markdown (800+ woorden). Gebruik \\n voor newlines in de body_markdown string.
...
{{
  ...
  "faq": [{{"q": "...", "a": "..."}}],
  "body_markdown": "## Inleiding\\n\\n..."
}}

BELANGRIJK: Alleen kale JSON output, géén ```json fences. Echte prijzen. Minimaal 5 tools. Minimaal 800 woorden body. Gebruik \\n voor newlines in body_markdown."""
